...
class SuperFormer(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=2, in_chans=1,
                 embed_dim=48, depths=[6, 6, 6], num_heads=[6, 6, 6, 6],
    ...
    def forward_features(self, x, x_feat):
        if self.patch_size>1:
            ...
            # compute patch-grid size from the actual input tensor so it's not fixed
            # to the model's init `img_size`. Use the PatchEmbed3D's patch_size
            # (already converted to a 3-tuple) to compute floor division.
            
            # if hasattr(self.patch_embed_volume, 'patch_size'):
            #     ps = self.patch_embed_volume.patch_size
            # else:
            #     ps = to_3tuple(self.patch_size)
            # H = x.shape[2] // ps[0]
            # W = x.shape[3] // ps[1]
            # D = x.shape[4] // ps[2]
            # x_size = (H, W, D)
